class DailyFoodRecordsController < ApplicationController
  ...
  def show
    ...
    @user = User.find(params[:user_id])
    if(@daily_food_record.nil? || @daily_food_record.food.nil?)
      render 'daily_food_records/show_empty'
      return
    end
    respond_to do |format|
  ...
  def create
    ...
    respond_to do |format|
      if @daily_food_record.save
        format.html { redirect_to [@user, @daily_food_record], notice: 'Daily food record was successfully created.' }
        format.json { render json: [@user, @daily_food_record], status: :created, location: @daily_food_record }
      else
  ...
  def update
    ...
    respond_to do |format|
      if @daily_food_record.update_attributes(params[:daily_food_record])
        format.html { redirect_to [@user, @daily_food_record], notice: 'Daily food record was successfully updated.' }
